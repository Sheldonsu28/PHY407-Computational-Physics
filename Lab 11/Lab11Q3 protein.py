...
from pylab import figure, plot, show, xlim, ylim, title, legend, xlabel, ylabel, savefig, errorbar
import numpy as np
...
for j in range(n):
    ...
    if monomer_array[new_monomer_pos[0], new_monomer_pos[1]] == 0:
        ...
        if i == 0:
            ...
            if dist(new_monomer_pos, monomer_coords[i - 1, :]) < 1.1 and dist(new_monomer_pos,
...
for i in np.arange(T_steps):
    mean_energy.append(np.mean(energy_array[int(i * n / T_steps):int((i + 1) * n / T_steps)]))
    std_energy.append(np.std(energy_array[int(i * n / T_steps):int((i + 1) * n / T_steps)]))
    temperature.append(0.5 + i * 0.5)
print(mean_energy)
print(temperature)
"""
figure()
...
"""
figure()
title('Temperature=%.1f, Protein length=%d' % (10, N))
errorbar(np.flip(temperature), mean_energy, yerr=std_energy)
xlim((10, 0))
xlabel('Temperature')
ylabel('Mean Energy')
savefig('energy_vs_temperature_T%d_N%d_n%d.png' % (10, N, n))
"""
figure()
...
print('Energy averaged over last half of simulations is: %.2f' % mean(energy_array[int (n / 4):]))
